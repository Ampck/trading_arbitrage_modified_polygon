...
// -- .ENV VALUES HERE -- //
const arbFor = process.env.ARB_FOR // This is the address of token we are attempting to arbitrage (TKN_1)
const arbAgainst = process.env.ARB_AGAINST // TKN_2
...
const checkPrice = async (_exchange, _token0, _token1) => {
  ...
  const priceDifference = (((uFPrice - sFPrice) / sFPrice) * 100).toFixed(units)
...
const determineDirection = async (_priceDifference) => {
  ...
  if (_priceDifference >= difference) {
    ...
    console.log('Difference not high enough to initiaze arbitrage attempt:')
    console.log(`Percentage Difference: ${_priceDifference}%`)
    console.log(`Minimum Difference: ${Number(difference).toFixed(units)}%`)
    return null
  }
}
...
const determineProfitability = async (_routerPath, _token0Contract, _token0, _token1) => {
  ...
  try {
    ...
    console.log(`Estimated amount of TKN_1 needed to buy enough TKN_2 on ${exchangeToBuy}\t\t| ${ethers.formatUnits(estimate[0], 'ether')}`)
    console.log(`Estimated amount of TKN_1 returned after swapping TKN_2 on ${exchangeToSell}\t| ${ethers.formatUnits(result[1], 'ether')}\n`)
    ...
    const data = {
      ...
      '-': {},
      'TKN_1 Balance BEFORE': wethBalanceBefore,
      'TKN_1 Balance AFTER': wethBalanceAfter,
      'TKN_1 Gained/Lost': wethBalanceDifference,
      '-': {},
...
const executeTrade = async (_routerPath, _token0Contract, _token1Contract) => {
  ...
  const data = {
    ...
    '-': {},
    'TKN_1 Balance BEFORE': ethers.formatUnits(tokenBalanceBefore, 'ether'),
    'TKN_1 Balance AFTER': ethers.formatUnits(tokenBalanceAfter, 'ether'),
    'TKN_1 Gained/Lost': ethers.formatUnits(tokenBalanceDifference.toString(), 'ether'),
    '-': {},
